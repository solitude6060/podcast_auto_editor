...
- PR-X2.1：為 `faster-whisper-local` ASR provider 加入 `BELLE-2/Belle-whisper-large-v3-zh` 的 env-gated 真實載入整合測試，並在 `docs/walkthroughs/real-podcast-ep1.md` 加入操作者使用說明。預設情況下測試會自動跳過；設 `PAE_BELLE_REAL=1 PAE_BELLE_REAL_AUDIO=<path>` 方可實際執行模型載入路徑。不變更任何公開 CLI 介面。
