...
- Review dashboard 新增單一操作的詳情 endpoint：`GET /api/operation/<id>`，以及狀態 endpoint 的篩選參數 `GET /api/status?filter=<type>`，可以先掃完所有 silence-cut 再處理 retake-cut。瀏覽器鍵盤快捷鍵：`a` accept、`r` reject、`u` undo、`j`/`k` 跳到下一個待審。游標在 reviewer / note 輸入欄時快捷鍵會自動暫停。
